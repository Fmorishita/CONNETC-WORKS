...
-- All Ops tables are prefixed "ops_" so they NEVER collide with other apps
-- sharing this Supabase project. Only "leads" is shared (on purpose).
...
-- ---- leads: extend the existing shared table for field operations ----
...
-- ============================ PHASE 1 TABLES ============================
create table if not exists public.ops_visits (
...
create table if not exists public.ops_daily_routes (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.ops_route_stops (
  id uuid primary key default gen_random_uuid(),
  daily_route_id uuid references public.ops_daily_routes(id) on delete cascade,
  visit_id uuid references public.ops_visits(id) on delete cascade,
...
create table if not exists public.ops_business_types (
...
create table if not exists public.ops_quote_line_items (
  id uuid primary key default gen_random_uuid(),
  quote_id uuid references public.ops_quotes(id) on delete cascade,
...
create table if not exists public.ops_quote_templates (
  id uuid primary key default gen_random_uuid(),
...
create table if not exists public.ops_follow_ups (
  id uuid primary key default gen_random_uuid(),
  quote_id uuid references public.ops_quotes(id) on delete cascade,
  lead_id uuid references public.leads(id) on delete set null,
...
create table if not exists public.ops_project_materials (
  id uuid primary key default gen_random_uuid(),
  project_id uuid references public.ops_projects(id) on delete cascade,
  visit_id uuid references public.ops_visits(id) on delete set null,
...
do $$ declare t text;
begin
  foreach t in array array['ops_visits','ops_daily_routes','ops_settings','ops_quotes','ops_follow_ups','ops_projects','ops_project_materials','leads']
  loop
...
do $$ declare t text;
begin
  foreach t in array array['ops_visits','ops_daily_routes','ops_route_stops','ops_settings','ops_team_members',
    'ops_business_types','ops_services','ops_activity_log','ops_quotes','ops_quote_line_items','ops_quote_templates',
    'ops_follow_ups','ops_projects','ops_project_materials']
  loop
...
insert into public.ops_team_members (name, role) values ('Omar','Owner / Technician') on conflict do nothing;
insert into public.ops_business_types (name) values
...
insert into public.ops_quote_templates (template_name, service_category, default_title, default_description) values
...
-- ============================================================================
-- OPTIONAL cleanup of the FIRST (un-prefixed) attempt. Run these ONE BY ONE
-- and ONLY if you are sure the table is NOT used by another app in this project.
-- (Your pre-existing "projects" almost certainly belongs to another app — do
-- NOT drop that one. These were the empty tables created by the first schema.)
-- ----------------------------------------------------------------------------
