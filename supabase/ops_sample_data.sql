...
-- Run in Supabase → SQL Editor. Safe & re-runnable (cleans its own samples).
...
delete from public.project_materials where notes='SAMPLE';
delete from public.projects where notes='SAMPLE';
delete from public.follow_ups where notes='SAMPLE';
delete from public.quote_line_items where quote_id in (select id from public.quotes where quote_number like 'Q-SAMPLE-%');
delete from public.quotes where quote_number like 'Q-SAMPLE-%';
delete from public.visits where internal_notes='SAMPLE';
...
insert into public.quotes
  (quote_number,client_name,business_name,client_email,client_phone,project_address,quote_title,quote_description,service_category,
   subtotal,discount,tax,total,deposit_required,estimated_start_date,estimated_duration,valid_until,status,
   warranty_text,terms_text)
values
('Q-SAMPLE-001','Marcus Lee','Bay City Brewing Co','marcus@baycitybrew.test','619-555-0142','3760 Hancock St, San Diego, CA 92110',
 'Commercial Security Camera Installation','SAMPLE',
 'Video Surveillance',4160,0,0,4160,2080,current_date+5,'3–4 days',current_date+15,'Draft',
...
insert into public.quote_line_items (quote_id,item_name,description,quantity,unit,unit_price,total,sort_order)
...
insert into public.follow_ups (quote_id,client_name,business_name,follow_up_date,follow_up_type,status,suggested_message,notes)
...
from public.quotes where quote_number='Q-SAMPLE-001';

-- ---- 6) Project + materials ----
insert into public.projects (client_name,business_name,project_address,service_category,project_scope,start_date,estimated_completion_date,assigned_team,status,materials_status,notes)
...
insert into public.project_materials (project_id,item_name,quantity,supplier,status,notes)
